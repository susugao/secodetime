...
def save_data(user_input, emotion_label):
    ...
    new_data = pd.DataFrame([[now, user_input, emotion_label]], columns=["時間", "學生輸入", "辨識情緒"])
...
if final_text:
    ...
    # B. 視覺與對話回饋
    fb = feedback_dict.get(label, {"msg": ["謝謝你告訴我。"], "action": None})
    st.divider()
...
with st.sidebar:
    ...
    if st.checkbox("查看學生紀錄清單"):
        if os.path.exists("student_logs.csv"):
            ...
            st.download_button(label="📥 下載報表 (CSV)", data=csv_data, file_name=f"情緒紀錄_{datetime.now().strftime('%m%d')}.csv")
        else:
            st.info("尚無紀錄。")
